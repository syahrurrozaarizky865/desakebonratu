...
export const AdminDashboard: React.FC = () => {
  ...
  const saveEditor = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (type === 'bumdes') {
      const item = { nama: data.nama, jenis_usaha: data.jenis_usaha, deskripsi: data.deskripsi, alamat: data.alamat, kontak: data.kontak, pemilik: data.pemilik, gambar: savedData.gambar, latitude: data.latitude, longitude: data.longitude };
          ...
          {editor && (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-sm">
              <form noValidate onSubmit={saveEditor} className="w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-2xl bg-white p-6 shadow-2xl dark:bg-slate-900 space-y-4">
                ...
                {editor.type === 'bumdes' && <><input required value={editor.data.nama} onChange={(e) => updateEditor('nama', e.target.value)} placeholder="Nama BUMDes" className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm dark:border-slate-700 dark:bg-slate-800" /><input required value={editor.data.jenis_usaha} onChange={(e) => updateEditor('jenis_usaha', e.target.value)} placeholder="Jenis usaha" className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm dark:border-slate-700 dark:bg-slate-800" /><textarea required rows={4} value={editor.data.deskripsi} onChange={(e) => updateEditor('deskripsi', e.target.value)} placeholder="Deskripsi BUMDes" className="w-full resize-none rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm dark:border-slate-700 dark:bg-slate-800" /><div className="grid grid-cols-2 gap-3"><input required value={editor.data.alamat} onChange={(e) => updateEditor('alamat', e.target.value)} placeholder="Alamat" className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm dark:border-slate-700 dark:bg-slate-800" /><input required value={editor.data.kontak} onChange={(e) => updateEditor('kontak', e.target.value)} placeholder="Kontak" className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm dark:border-slate-700 dark:bg-slate-800" /></div><div className="grid grid-cols-2 gap-3"><input required type="number" step="any" value={editor.data.latitude} onChange={(e) => updateEditor('latitude', e.target.value)} placeholder="Latitude GPS" className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm dark:border-slate-700 dark:bg-slate-800" /><input required type="number" step="any" value={editor.data.longitude} onChange={(e) => updateEditor('longitude', e.target.value)} placeholder="Longitude GPS" className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm dark:border-slate-700 dark:bg-slate-800" /></div><input required value={editor.data.pemilik} onChange={(e) => updateEditor('pemilik', e.target.value)} placeholder="Pengelola / pemilik" className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm dark:border-slate-700 dark:bg-slate-800" /><label className="block cursor-pointer rounded-xl border-2 border-dashed border-emerald-300 bg-emerald-50/60 p-4 text-center dark:border-emerald-800 dark:bg-emerald-950/30">{editorPreview ? <img src={editorPreview} alt="Pratinjau foto BUMDes" className="mx-auto max-h-36 rounded-lg object-contain" /> : <><Upload className="mx-auto mb-1 h-6 w-6 text-emerald-600" /><p className="text-sm font-bold text-emerald-800 dark:text-emerald-300">Pilih foto BUMDes</p><p className="mt-1 text-[11px] text-slate-500">JPG, PNG, atau WebP. Maksimal 2 MB.</p></>}<input required={!editor.data.gambar} type="file" accept="image/jpeg,image/png,image/webp" className="sr-only" onChange={(event) => handleEditorFileChange(event.target.files?.[0])} /></label>{editorFile && <p className="text-xs text-slate-500">File: {editorFile.name}</p>}</>}
